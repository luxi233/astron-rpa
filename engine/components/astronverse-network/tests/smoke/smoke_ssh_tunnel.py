...
print("== 3. 端口默认值 ==")
SshTunnel.open_ssh_tunnel(
    ssh_host="j", ssh_port="", ssh_user="u", password="p", remote_host="r", remote_port=80, local_port=""
)
...
expect_err(
    "目标地址为空(首次format,全文)",
    lambda: SshTunnel.open_ssh_tunnel(
        ssh_host="j", ssh_port=22, ssh_user="u", password="p", remote_host="", remote_port=80
    ),
    "目标服务地址为空",
)
expect_err(
    "目标端口为0(前缀,模板已污染)",
    lambda: SshTunnel.open_ssh_tunnel(
        ssh_host="j", ssh_port=22, ssh_user="u", password="p", remote_host="r", remote_port=0
    ),
    "SSH隧道创建失败",
)
expect_err(
    "密钥模式缺私钥路径(前缀)",
    lambda: SshTunnel.open_ssh_tunnel(
        ssh_host="j", ssh_port=22, ssh_user="u", login_mode=SshLoginMode.KEY, remote_host="r", remote_port=80
    ),
    "SSH隧道创建失败",
)
expect_err(
    "私钥文件不存在(FILE_EXIST首次,全文)",
    lambda: SshTunnel.open_ssh_tunnel(
        ssh_host="j",
        ssh_port=22,
        ssh_user="u",
        login_mode=SshLoginMode.KEY,
        key_path="/tmp/no_such_key.pem",
        remote_host="r",
        remote_port=80,
    ),
    "不存在",
...
expect_err(
    "start失败包装(前缀)",
    lambda: SshTunnel.open_ssh_tunnel(
        ssh_host="j", ssh_port=22, ssh_user="u", password="p", remote_host="r", remote_port=80
    ),
    "SSH隧道创建失败",
)
...
expect_err(
    "关闭无效对象(无stop方法)", lambda: SshTunnel.close_ssh_tunnel(tunnel_instance="not_a_tunnel"), "SSH隧道关闭失败"
)
