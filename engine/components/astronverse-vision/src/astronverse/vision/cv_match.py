...
class AnchorMatch:
    ...
    def check_if_multiple_elements(image, element, match_similarity):
        ...
        # 通道权重不一致, 彩色元素下灰度图错位导致匹配失准
        gray = cv2.cvtColor(image, cv2.COLOR_RGB2GRAY)
        # gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        # L3修复: count 原仅在 if 块内赋值, element 为 None 时 NameError
        count = 0
        ...
        if element is not None:
            ...
            if match_box is None:
                ...
                if fallback_max_val >= fallback_threshold:
                    ...
                    logger.info("锚点范围未找到，已通过全局匹配兜底")
                else:
                    logger.info("全局兜底也未找到目标元素，可能已移除或变化过大")
        else:
            logger.info("请选择目标元素")
            return None
