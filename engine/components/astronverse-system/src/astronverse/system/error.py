...
ASSERT_FAILED_FORMAT: ErrorCode = ErrorCode(BizCode.LocalErr, _("断言失败，流程已中断"))
FILE_BASE64_ERROR_FORMAT: ErrorCode = ErrorCode(BizCode.LocalErr, _("Base64编码文件失败") + ": {}")
FILE_MD5_ERROR_FORMAT: ErrorCode = ErrorCode(BizCode.LocalErr, _("计算文件MD5失败") + ": {}")
FILE_EXPAND_ENV_ERROR_FORMAT: ErrorCode = ErrorCode(BizCode.LocalErr, _("展开路径环境变量失败") + ": {}")
FILE_LOCK_CHECK_ERROR_FORMAT: ErrorCode = ErrorCode(BizCode.LocalErr, _("检查文件占用失败") + ": {}")
PRINTER_ERROR_FORMAT: ErrorCode = ErrorCode(BizCode.LocalErr, _("打印机操作失败") + ": {}")
PRINTER_NOT_FOUND_FORMAT: ErrorCode = ErrorCode(BizCode.LocalErr, _("未发现 {} 打印机，请检查打印机名称"))
PRINTER_NOT_SUPPORTED_FORMAT: ErrorCode = ErrorCode(BizCode.LocalErr, _("当前系统不支持打印机操作，仅在 Windows 可用"))
SCREEN_ERROR_FORMAT: ErrorCode = ErrorCode(BizCode.LocalErr, _("屏幕设置失败") + ": {}")
SCREEN_NOT_SUPPORTED_FORMAT: ErrorCode = ErrorCode(BizCode.LocalErr, _("当前系统不支持屏幕设置，仅在 Windows 系统可用"))
DEVICE_ERROR_FORMAT: ErrorCode = ErrorCode(BizCode.LocalErr, _("系统设备操作失败") + ": {}")
DEVICE_NOT_SUPPORTED_FORMAT: ErrorCode = ErrorCode(BizCode.LocalErr, _("当前系统不支持该操作，仅在 Windows 系统可用"))
