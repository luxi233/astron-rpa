...
import { BrowserElementVisible, BrowserTextExist, Catch, CvImageExist, CvImageExistEnd, CvOcrTextExist, FileExist, FolderExist, ForBrowserSimilar, ForDataTableLoop, ForDict, ForEnd, ForExcelContent, ForList, ForStep, Group, GroupEnd, If, IfEnd, IfMulti, Infinite, Try, TryEnd, While, WinContainElement, WindowExist, WinLoopSimilar } from '@/views/Arrange/config/atomKeyMap'
...
export const useFlowStore = defineStore('flow', () => {
  ...
  const generateContactMap = (list: RPA.Atom[]) => {
    ...
    list.forEach((i) => {
      ...
      if (i.key === Group)
        groupList.push(i)
      else if ([ForStep, ForDict, ForList, ForExcelContent, ForBrowserSimilar, ForDataTableLoop, WinLoopSimilar, While, Infinite].includes(i.key))
        forList.push(i)
